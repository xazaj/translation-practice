...
const store = {
  ...
  saveToLocalStorage() {
    const dataToSave = {
      ...
      learningStats: this.state.learningStats,
  ...
  loadFromLocalStorage() {
    ...
    if (savedState) {
      try {
        ...
        // 合并状态
        this.state.questions = parsedState.questions || [];
        this.state.currentPage = parsedState.currentPage || 1;
        this.state.learningStats = { ...this.state.learningStats, ...parsedState.learningStats };
        ...
        // 转换日期字符串为Date对象
        if (this.state.learningStats.lastStudyDate) {
          this.state.learningStats.lastStudyDate = new Date(this.state.learningStats.lastStudyDate);
        }
      } catch (error) {
