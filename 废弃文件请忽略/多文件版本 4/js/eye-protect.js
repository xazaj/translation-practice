...
function initEyeProtection() {
  ...
  if (localStorage.getItem('eye_protect_mode') === 'true') {
    ...
    eyeProtectToggle.classList.add('active'); // 添加激活状态类
  }
  ...
  eyeProtectToggle.addEventListener('click', function() {
    ...
    // 更新激活状态
    this.classList.toggle('active', isEyeProtectMode);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const eyeProtectToggle = document.getElementById('eyeProtectToggle');
  if (eyeProtectToggle) {
    console.log('找到护眼模式按钮');
    // 确保图标显示
    const eyeIcon = eyeProtectToggle.querySelector('.eye-icon');
    if (eyeIcon) {
      console.log('找到护眼模式图标');
      // 确保图标内容
      eyeIcon.textContent = localStorage.getItem('eye_protect_mode') === 'true' ? '👁️‍🗨️' : '👁️';
    } else {
      console.error('未找到护眼模式图标');
    }
  } else {
    console.error('未找到护眼模式按钮');
  }
